...
export default function ProductFilters({
  q, setQ, type, setType, status, setStatus, sort, setSort, order, setOrder, onRefresh,
}: Props) {
  return (
    <div className="flex flex-wrap items-end gap-3">
      ...
      <div>
        ...
        <select className="rounded-lg border px-3 py-2" value={type} onChange={(e) => setType(e.target.value as any)}>
      ...
      <div>
        ...
        <select className="rounded-lg border px-3 py-2" value={sort} onChange={(e) => setSort(e.target.value as any)}>
      ...
      <div>
        ...
        <select className="rounded-lg border px-3 py-2" value={order} onChange={(e) => setOrder(e.target.value as any)}>
          <option value="asc">Asc</option>
          <option value="desc">Desc</option>
        </select>
      </div>

      <button onClick={onRefresh} className="rounded-lg border px-3 py-2">Apply</button>
    </div>
  );
}